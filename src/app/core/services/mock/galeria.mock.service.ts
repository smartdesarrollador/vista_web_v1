...
import { GaleriaItem, GaleriaCategory } from '../../models/galeria-item.interface';
import { MockDataService } from './mock-data.service';

@Injectable({
  providedIn: 'root'
})
export class GaleriaMockService {

  private categories: GaleriaCategory[] = [
  ...
  constructor(private mockData: MockDataService) {
    this.initializeGaleriaItems();
  }

  private initializeGaleriaItems(): void {
    this.galeriaItems = [
      {
        id: 1,
  ...
  getAllItems(): Observable<GaleriaItem[]> {
    return of(this.galeriaItems.filter(item => item.isActive));
  }

  getItemById(id: number): Observable<GaleriaItem | undefined> {
    const item = this.galeriaItems.find(p => p.id === id);
    return of(item);
  }

  getItemsByCategory(category: string): Observable<GaleriaItem[]> {
    const items = this.galeriaItems.filter(p => p.category === category && p.isActive);
    return of(items);
  }

  getFeaturedItems(): Observable<GaleriaItem[]> {
    const featured = this.galeriaItems.filter(p => p.isFeatured && p.isActive);
    return of(featured);
  }

  getRecentItems(limit: number = 6): Observable<GaleriaItem[]> {
    const recent = this.galeriaItems
      .filter(p => p.isActive)
  ...
  getCategories(): Observable<GaleriaCategory[]> {
